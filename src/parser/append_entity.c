...
t_entity	*get_core_entity_data(t_entity *ent, char **data)
{
	ent->enabled = true;
...
// t_entity	*get_enemy_entity_data(t_entity *ent, char **data)
// {
// 	char		**temp;
// 	size_t		i;
// 	size_t		j;
...
// t_entity *get_enemy_entity_data(t_entity *new_entity, char **temp)
// {
	// char **temp2;
	// size_t	i;
	// size_t	j;
...
t_entity	*get_other_entity_data(t_entity *ent, char **data, uint8_t type)
{
	char	p[2];
	size_t	i;
...
t_entity	*append_entity(t_entity *entities, char *line, uint8_t type)
{
	...
	// new_entity = get_core_entity_data(new_entity, temp);
	...
	if (type == CONT_ENEMY)
	{
		...
		// new_entity = get_enemy_entity_data(new_entity, temp);
	...
	else if (type == CONT_OBJECT || type == CONT_ITEM)
	{
		...
		// new_entity = get_other_entity_data(new_entity, temp, type);
