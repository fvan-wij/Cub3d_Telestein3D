...
void	resolve_blood_particles(mlx_image_t *img, t_blood *blood_particles)
{
	int i = 0;
	int j = 0;

	if (!blood_particles->b_timer)
	{
		fade_blood(img);
		return (init_wall_destruction_fx(blood_particles));
	}
	while (i < MAX_BLOOD_PARTICLES)
	{
		t_particle *particle = &blood_particles->particles[i];
		if (blood_particles->b_timer)
		{
...
void	cbd_render(t_app *cbd)
{
	...
	resolve_blood_particles(cbd->render.hud->img[HUD_OVERLAY], &cbd->render.blood_particles);
}
