...
void	play_single_animation(float dt, t_weapon *weapon)
{
	...
	current_frame = (int) (weapon->use_anim->timer / weapon->use_anim->frames[0].duration);
...
{
		current_weapon->use_anim->loop = false;
		if (current_weapon->use_anim->frames[0].img)
			current_weapon->use_anim->frames[0].img->enabled = false;
		if (current_weapon->use_anim->frames[1].img)
			current_weapon->use_anim->frames[1].img->enabled = false;
		if (current_weapon->use_anim->frames[2].img)
			current_weapon->use_anim->frames[2].img->enabled = false;
		if( current_weapon->img->enabled)
			current_weapon->img->enabled = true;
		if (current_weapon->use_anim->frames[1].img)
			current_weapon->use_anim->frames[1].img->instances->x = current_weapon->use_anim->reset_x;
		// if ()
		current_weapon->use_anim->current_x = current_weapon->use_anim->reset_x;
}
